...
if __name__ == "__main__":
    # 訓練參數設置
    epoch = 300
    traning_size = 24000 # diff: 14683 ；diff_2: 29366 # same: 23000 # diff:18313 diff_3 47612
    batch_size = 1000
    data_set_size = traning_size

    # 輸入模擬資料
    path1 = 'motor_dataset/Motor_x_data_combine_diff_3.txt'
    path2 = 'motor_dataset/Motor_P_data_combine_diff_3.txt' 
    ...
    # x_optimizer = optim.AdamW(model.parameters(), lr=0.001, weight_decay=0.01)
    # x_optimizer = optim.NAdam(x_tcn_model.parameters(), lr=0.001)
    ...
    for epoch in range(epoch + 1):
        ...
        for i in range(0, traning_size, batch_size):
            ...
            # 計算損失
            # x_target = torch.tensor(cp.array(x_cmd)[1:,:], dtype=torch.float32).to(device)
            ...
            norm1 = max1 - min1
            x_tcn_output[:, 0] = (x_tcn_output[:, 0]-min1)/norm1
            x_target[:, 0] = (x_target[:, 0]-min1)/norm1

            min2 = cp.min(cp.array(x_k_update_data)[:, 1].get())
            max2 = cp.max(cp.array(x_k_update_data)[:, 1].get())
            norm2 = max2 - min2
            x_tcn_output[:, 1] = (x_tcn_output[:, 1]-min2)/norm2
            x_target[:, 1] = (x_target[:, 1]-min2)/norm2

            min3 = cp.min(cp.array(x_k_update_data)[:, 2].get())
            max3 = cp.max(cp.array(x_k_update_data)[:, 2].get())
            norm3 = max3 - min3
            x_tcn_output[:, 2] = (x_tcn_output[:, 2]-min3)/norm3
            x_target[:, 2] = (x_target[:, 2]-min3)/norm3
            
            x_loss = x_loss_fn(x_tcn_output[1:batch_size, 1:3], x_target[i+1:i+batch_size,1:3]) # 隨意嘗試
            # x_loss = x_loss_fn(x_tcn_output[1:batch_size, 0:3], x_target[i+1:i+batch_size,0:3]) # 考慮三者的loss 
    ...
    # x result儲存模型
    full_path = "motor/motor_model/x_tcn_model_fea{}_ker{}_num{}_epo{}.pth".format(input_size, kernel_size, num_channels, total_epoch)
    print(full_path)
